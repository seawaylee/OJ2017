package nowcoder.wy_2017_spring._10_words;
...
public class Main
{
    public static void main(String[] args)
    {
        ...
        for (int i = 0; i < nw; i++)
        {
            testWords.add(sc.next());
        }
        for (int i = 0; i < nd; i++)
        {
            dictWords.add(sc.next());
        }
        long scores = 0;
        for (String testWord : testWords)
        {
            if (dictWords.contains(testWord))
            {
                scores += Math.pow(testWord.length(), 2);
            }
        }
